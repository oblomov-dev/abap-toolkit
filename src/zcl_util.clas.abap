...
*  license: MIT.
    ...
    CLASS-METHODS url_param_create_url
      IMPORTING
        !t_params     TYPE zif_types=>ty_t_name_value
      RETURNING
        VALUE(result) TYPE string.
    ...
    CLASS-METHODS url_param_get_tab
      IMPORTING
        i_val            TYPE clike
      RETURNING
        VALUE(rt_params) TYPE zif_types=>ty_t_name_value.
    ...
    CLASS-METHODS filter_get_token_range_mapping
      RETURNING
        VALUE(result) TYPE zif_types=>ty_t_name_value.
...
CLASS zcl_util IMPLEMENTATION.
  ...
  METHOD db_load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF zt_02 WITH EMPTY KEY.
  ...
  METHOD json_parse.
    TRY.
        ...
        zcl_ajson=>parse( val )->to_abap(
          EXPORTING
            iv_corresponding = abap_true
          IMPORTING
            ev_container = data ).

      CATCH zcx_ajson_error INTO DATA(x).
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST zif_ajson( zcl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE zcx_util_error EXPORTING val = v.
    ENDIF.
