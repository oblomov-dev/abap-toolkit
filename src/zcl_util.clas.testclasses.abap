...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_boolean_check.
    ...
    cl_abap_unit_assert=>assert_equals(
        act = zcl_util=>boolean_check_by_data( lv_bool )
        exp = abap_true ).
    ...
    cl_abap_unit_assert=>assert_equals(
        act = zcl_util=>boolean_check_by_data( abap_true )
        exp = abap_true ).

    cl_abap_unit_assert=>assert_equals(
        act = zcl_util=>boolean_check_by_data( abap_false )
        exp = abap_true ).
  ...
  METHOD rtti_check_ref_data.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = zcl_util=>rtti_check_ref_data( lr_data )
      exp = abap_true ).

    cl_abap_unit_assert=>assert_equals(
      act = zcl_util=>rtti_check_ref_data( lv_test )
      exp = abap_false ).
  ...
  METHOD test_time_get_timestampl.
    ...
    DATA(lv_time2) = zcl_util=>time_substract_seconds(
         time    = lv_time
         seconds = 60 * 60 * 4 ).
  ...
  METHOD test_func_get_user_tech.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = sy-uname
      exp = zcl_util=>context_get_user_tech( ) ).
  ...
  METHOD test_url_param_create_url.
    ...
    DATA(lt_param) = zcl_util=>url_param_get_tab( `https://url.com/rvice_for_ui?sap-client=100&app_start=z2ui5_cl_app_hello_world` ).
  ...
  METHOD test_rtti_check_type_kind.
    ...
    DATA lr_string TYPE REF TO string.
    cl_abap_unit_assert=>assert_equals(
        act = zcl_util=>rtti_check_type_kind_dref( lv_string )
        exp = abap_false ).


    CREATE DATA lr_string.
    cl_abap_unit_assert=>assert_equals(
        act = zcl_util=>rtti_check_type_kind_dref( lr_string )
        exp = abap_true ).
  ...
  METHOD test_get_token_t_by_r_t.
    ...
    DATA(lt_range) = VALUE zcl_util=>ty_t_range(
