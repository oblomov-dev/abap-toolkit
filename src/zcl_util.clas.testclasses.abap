...
CLASS ltcl_unit_test_abap_api IMPLEMENTATION.
  ...
  METHOD test_raise_error.
    ...
    TRY.
      ...
      CATCH zcx_util_error INTO DATA(lx).
...
CLASS ltcl_unit_test IMPLEMENTATION.
  METHOD test_boolean_check.
    ...
    cl_abap_unit_assert=>assert_true( zcl_util=>boolean_check_by_data( lv_bool ) ).

    lv_bool = xsdbool( 1 = 2 ).
    cl_abap_unit_assert=>assert_true( zcl_util=>boolean_check_by_data( lv_bool ) ).

    cl_abap_unit_assert=>assert_true( zcl_util=>boolean_check_by_data( abap_true ) ).

    cl_abap_unit_assert=>assert_true( zcl_util=>boolean_check_by_data( abap_false ) ).
  ...
  METHOD test_sql_get_by_string.
    ...
    DATA(lv_test) = ``.
    DATA(ls_sql) = zcl_util=>filter_get_sql_by_sql_string( lv_test ) ##NEEDED.

  ENDMETHOD.

  METHOD test_create.

    DATA(lo_test) = NEW zcl_util( ) ##NEEDED.

  ENDMETHOD.

  METHOD test_rtti_get_classname_by_ref.

    DATA(lo_test) = NEW zcl_util( ).
    DATA(lv_name) = zcl_util=>rtti_get_classname_by_ref( lo_test ).
    cl_abap_unit_assert=>assert_equals( exp = `Z2UI5_CL_UTIL`
                                        act = lv_name ).

    DATA(lo_test2) = NEW ltcl_test_app( ).
    DATA(lv_name2) = zcl_util=>rtti_get_classname_by_ref( lo_test2 ).
  ...
  METHOD test_check_bound_a_not_inital.
    ...
    cl_abap_unit_assert=>assert_true( zcl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lv_test.
    cl_abap_unit_assert=>assert_false( zcl_util=>check_bound_a_not_inital( lr_test ) ).
    CLEAR lr_test.
    cl_abap_unit_assert=>assert_false( zcl_util=>check_bound_a_not_inital( lr_test ) ).
  ...
  METHOD test_check_unassign_inital.
    ...
    cl_abap_unit_assert=>assert_false( zcl_util=>check_unassign_inital( lr_test ) ).

    CLEAR lv_test.
    cl_abap_unit_assert=>assert_true( zcl_util=>check_unassign_inital( lr_test ) ).
  ...
  METHOD rtti_check_ref_data.
    ...
    cl_abap_unit_assert=>assert_true( zcl_util=>rtti_check_ref_data( lr_data ) ).

    cl_abap_unit_assert=>assert_false( zcl_util=>rtti_check_ref_data( lv_test ) ).
  ...
  METHOD conv_copy_ref_data.
    ...
    DATA(lr_test2) = zcl_util=>conv_copy_ref_data( lr_data ).
  ...
  METHOD test_boolean_abap_2_json.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `{ABCD}`
                                        act = zcl_util=>boolean_abap_2_json( `{ABCD}` ) ).

  ENDMETHOD.

  METHOD test_time_get_timestampl.

    DATA(lv_time) = zcl_util=>time_get_timestampl( ).

    DATA(lv_time2) = zcl_util=>time_substract_seconds( time    = lv_time
                                                            seconds = 60 * 60 * 4 ).
  ...
  METHOD test_time_substract_seconds.
    ...
    DATA(lv_time) = zcl_util=>time_get_timestampl( ).
    DATA(lv_time2) = zcl_util=>time_get_timestampl( ).
  ...
  METHOD test_c_trim_lower.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `jsadfhhs`
                                        act = zcl_util=>c_trim_lower( ` JsadfHHs  ` ) ).
  ...
  METHOD test_c_trim_upper.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `JSADFHHS`
                                        act = zcl_util=>c_trim_upper( ` JsadfHHs  ` ) ).

  ENDMETHOD.

  METHOD test_func_get_user_tech.

    cl_abap_unit_assert=>assert_equals( exp = zcl_util=>context_get_user_tech( )
                                        act = sy-uname ).

    cl_abap_unit_assert=>assert_not_initial( zcl_util=>context_get_user_tech( ) ).
  ...
  METHOD test_x_check_raise.
    ...
    TRY.
        zcl_util=>x_check_raise( xsdbool( 1 = 1 ) ).
        cl_abap_unit_assert=>fail( ).
      CATCH cx_root.
    ENDTRY.

    TRY.
        zcl_util=>x_check_raise( xsdbool( 1 = 3 ) ).
      CATCH cx_root.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ...
  METHOD test_trans_json_any_2__w_struc.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `{"selected":false,"title":"test","value":""}`
                                        act = zcl_util=>json_stringify( ls_row ) ).

  ENDMETHOD.

  METHOD test_url_param_create_url.

    DATA(lt_param) = zcl_util=>url_param_get_tab(
                         `https://url.com/rvice_for_ui?sap-client=100&app_start=z2ui5_cl_app_hello_world` ).
    DATA(lv_url) = zcl_util=>url_param_create_url( lt_param ).
  ...
  METHOD test_url_param_get.
    ...
    DATA(lv_param) = zcl_util=>url_param_get(
                         val = `app_start`
  ...
  METHOD test_url_param_set.
    ...
    DATA(lv_param) = zcl_util=>url_param_set(
                         name  = `app_start`
  ...
  METHOD test_x_check_raise_not.
    ...
    TRY.
        zcl_util=>x_check_raise( xsdbool( 1 = 2 ) ).
      CATCH zcx_util_error.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD test_rtti_get_type_name.
    ...
    DATA(lv_name) = zcl_util=>rtti_get_type_name( lv_xsdbool ).
  ...
  METHOD test_rtti_get_type_kind.
    ...
    DATA(lv_type_kind) = zcl_util=>rtti_get_type_kind( lv_string ).
    ...
    lv_type_kind = zcl_util=>rtti_get_type_kind( lr_string ).
  ...
  METHOD test_rtti_check_type_kind.
    ...
    DATA(lv_string) = VALUE string( ).
    DATA lr_string TYPE REF TO string.
    cl_abap_unit_assert=>assert_false( zcl_util=>rtti_check_type_kind_dref( lv_string ) ).

    CREATE DATA lr_string.
    cl_abap_unit_assert=>assert_true( zcl_util=>rtti_check_type_kind_dref( lr_string ) ).
  ...
  METHOD test_rtti_get_t_attri_by_obj.
    ...
    DATA(lt_attri) = zcl_util=>rtti_get_t_attri_by_oref( lo_obj ).
  ...
  METHOD test_rtti_get_t_comp_by_struc.
    ...
    DATA(lt_comp) = zcl_util=>rtti_get_t_attri_by_any( ls_row ).
  ...
  METHOD test_trans_xml_any_2__w_obj.
    ...
    DATA(lo_obj) = NEW ltcl_test_app( ).
    DATA(lv_xml) = zcl_util=>xml_stringify( lo_obj ).
  ...
  METHOD test_trans_xml_2_any__w_obj.
    ...
    CLEAR lo_obj.
    zcl_util=>xml_parse( EXPORTING xml = lv_xml
  ...
  METHOD test_trans_xml_any_2__w_data.
    ...
    DATA(lv_xml) = zcl_util=>xml_stringify( ls_row ).
  ...
  METHOD test_c_trim_horizontal_tab.
    ...
    IF zcl_util=>c_trim( |{ cl_abap_char_utilities=>horizontal_tab }|
                                && |JsadfHHs|
  ...
  METHOD test_get_token_t_by_r_t.
    ...
    DATA(lt_range) = VALUE zcl_util=>ty_t_range( ( sign = 'I' option = 'EQ' low = `table` high = `` )
     ).

    DATA(lt_result) = zcl_util=>filter_get_token_t_by_range_t( lt_range ).

    DATA(lt_exp) = VALUE zcl_util=>ty_t_token(
  ...
  METHOD test_rtti_get_t_attri_by_incl.
    ...
    DATA(lt_attri) = zcl_util=>rtti_get_t_attri_by_include( CAST #( lo_datadescr ) ).
