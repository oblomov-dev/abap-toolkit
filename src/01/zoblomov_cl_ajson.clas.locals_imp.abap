  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS split_path
      IMPORTING
        iv_path TYPE string
      RETURNING
        VALUE(rv_path_name) TYPE zoblomov_if_ajson_types=>ty_path_name.
...
CLASS lcl_utils IMPLEMENTATION.
  ...
  METHOD validate_array_index.
    ...
    IF NOT iv_index CO '0123456789'.
      zoblomov_cx_ajson_error=>raise( |Cannot add non-numeric key [{ iv_index }] to array [{ iv_path }]| ).
    ENDIF.
    rv_index = iv_index.
    IF rv_index = 0.
      zoblomov_cx_ajson_error=>raise( |Cannot add zero key to array [{ iv_path }]| ).
    ENDIF.
    ...
    CASE lo_type->type_kind.
      ...
      WHEN lif_kind=>table.
        ...
        IF lo_table_type->table_kind <> cl_abap_tabledescr=>tablekind_std.
          zoblomov_cx_ajson_error=>raise( 'Unsupported type of input table (must be standard table)' ).
        ENDIF.
        TRY.
          ...
          CATCH cx_root.
            zoblomov_cx_ajson_error=>raise( 'Error converting input table (should be string_table)' ).
        ENDTRY.
      WHEN OTHERS.
        zoblomov_cx_ajson_error=>raise( 'Unsupported type of input (must be char, string, string_table, or xstring)' ).
    ENDCASE.
...
CLASS lcl_json_parser DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zoblomov_if_ajson_types=>ty_nodes_tt
      RAISING
        zoblomov_cx_ajson_error.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ty_stack_tt TYPE STANDARD TABLE OF REF TO zoblomov_if_ajson_types=>ty_node.
    ...
    METHODS _parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zoblomov_if_ajson_types=>ty_nodes_tt
      RAISING
        zoblomov_cx_ajson_error cx_dynamic_check. " cx_sxml_error is not released on Steampunk #153
...
CLASS lcl_json_serializer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    CLASS-METHODS stringify
      IMPORTING
        it_json_tree TYPE zoblomov_if_ajson_types=>ty_nodes_ts
  ...
  PRIVATE SECTION.
    ...
    DATA mt_json_tree TYPE zoblomov_if_ajson_types=>ty_nodes_ts.
    DATA mv_keep_item_order TYPE abap_bool.
    ...
    METHODS stringify_node
      IMPORTING
        is_node TYPE zoblomov_if_ajson_types=>ty_node
      RAISING
        zoblomov_cx_ajson_error.
  ...
  METHOD stringify_node.
    ...
    CASE is_node-type.
      ...
      WHEN zoblomov_if_ajson_types=>node_type-object.
        lv_item = lv_item && '{'.
      WHEN zoblomov_if_ajson_types=>node_type-string.
        lv_item = lv_item && |"{ escape_string( is_node-value ) }"|.
      WHEN zoblomov_if_ajson_types=>node_type-boolean OR zoblomov_if_ajson_types=>node_type-number.
    ...
    IF mv_indent_step > 0
      AND ( is_node-type = zoblomov_if_ajson_types=>node_type-array OR is_node-type = zoblomov_if_ajson_types=>node_type-object )
    ...
    IF is_node-type = zoblomov_if_ajson_types=>node_type-array OR is_node-type = zoblomov_if_ajson_types=>node_type-object.
...
CLASS lcl_json_to_abap DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS to_abap
      IMPORTING
        it_nodes     TYPE zoblomov_if_ajson_types=>ty_nodes_ts
      CHANGING
    ...
    METHODS to_timestamp
      IMPORTING
        iv_value         TYPE zoblomov_if_ajson_types=>ty_node-value
      RETURNING
  ...
  PRIVATE SECTION.
    ...
    DATA mr_nodes TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.
    DATA mi_custom_mapping TYPE REF TO zoblomov_if_ajson_mapping.
    ...
    METHODS get_node_type
      IMPORTING
        is_node            TYPE zoblomov_if_ajson_types=>ty_node OPTIONAL " Empty for root
        is_parent_type     TYPE ty_type_cache OPTIONAL
...
CLASS lcl_json_to_abap IMPLEMENTATION.
  ...
  METHOD get_node_type.
    ...
    IF sy-subrc <> 0.
      ...
      CASE is_parent_type-type_kind.
        ...
        WHEN lif_kind=>struct_flat OR lif_kind=>struct_deep.
          ...
          IF sy-subrc <> 0.
            IF mv_corresponding = abap_false.
              zoblomov_cx_ajson_error=>raise( |Path not found| ).
            ELSE.
              CLEAR rs_node_type.
              RETURN.
            ENDIF.
        ...
        WHEN OTHERS.
          zoblomov_cx_ajson_error=>raise( |Unexpected parent type| ).
      ENDCASE.
  ...
  METHOD any_to_abap.
    ...
    DATA lx_ajson TYPE REF TO zoblomov_cx_ajson_error.
    DATA lx_root TYPE REF TO cx_root.
    DATA lr_target_field TYPE REF TO data.

    FIELD-SYMBOLS <n> TYPE zoblomov_if_ajson_types=>ty_node.
    ...
    TRY.
      ...
      " array_index because stringified index goes in wrong order [1, 10, 2 ...]
        ...
        " Validate node type
          ...
          " TODO maybe in future
            zoblomov_cx_ajson_error=>raise( 'Cannot assign to ref' ).
          ENDIF.
          ...
          CASE is_parent_type-type_kind.
            WHEN lif_kind=>table.
              ...
              " Does not affect anything actually but for integrity
                zoblomov_cx_ajson_error=>raise( 'Need index to access tables' ).
              ENDIF.
        ...
        " Process value assignment
          CASE <n>-type.
            WHEN zoblomov_if_ajson_types=>node_type-object.
              IF ls_node_type-type_kind <> lif_kind=>struct_flat AND
               ls_node_type-type_kind <> lif_kind=>struct_deep.
                zoblomov_cx_ajson_error=>raise( 'Expected structure' ).
              ENDIF.
              any_to_abap(
            ...
            WHEN zoblomov_if_ajson_types=>node_type-array.
              IF NOT ls_node_type-type_kind = lif_kind=>table.
                zoblomov_cx_ajson_error=>raise( 'Expected table' ).
              ENDIF.
              any_to_abap(
              iv_path         = <n>-path && <n>-name && '/'
              is_parent_type  = ls_node_type
          ...
          IF is_parent_type-tab_item_buf IS BOUND. " Indirect hint that table was sorted/hashed, see get_node_type.
            TRY.
                ...
                IF sy-subrc <> 0.
                  zoblomov_cx_ajson_error=>raise( 'Duplicate insertion' ).
                ENDIF.
      ...
      CATCH zoblomov_cx_ajson_error INTO lx_ajson.
  ...
  METHOD value_to_abap.
    ...
    IF is_node_type-type_kind CA lif_kind=>deep_targets.
      zoblomov_cx_ajson_error=>raise( |Unsupported target for value [{ is_node_type-type_kind }]| ).
    ENDIF.
    ...
    CASE is_node-type.
      ...
      WHEN zoblomov_if_ajson_types=>node_type-number.
        " TODO: check type ?
      ...
      WHEN zoblomov_if_ajson_types=>node_type-string.
        ...
        ENDIF.
      WHEN OTHERS.
        zoblomov_cx_ajson_error=>raise( |Unexpected JSON type [{ is_node-type }]| ).
    ENDCASE.
  ...
  METHOD to_date.
    ...
    IF sy-subrc <> 0.
      zoblomov_cx_ajson_error=>raise( 'Unexpected date format' ).
    ENDIF.
  ...
  METHOD to_timestamp.
    ...
    ELSE.
      ...
      IF sy-subrc <> 0.
        zoblomov_cx_ajson_error=>raise( 'Unexpected timestamp format' ).
      ENDIF.
    ...
    TRY.
      ...
      CATCH cx_parameter_invalid_range cx_parameter_invalid_type.
        zoblomov_cx_ajson_error=>raise( 'Unexpected error calculating timestamp' ).
    ENDTRY.
  ...
  METHOD to_time.
    ...
    IF sy-subrc <> 0.
      zoblomov_cx_ajson_error=>raise( 'Unexpected time format' ).
    ENDIF.
...
CLASS lcl_abap_to_json DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS convert
      IMPORTING
        iv_data            TYPE any
        is_prefix          TYPE zoblomov_if_ajson_types=>ty_path_name OPTIONAL
        iv_array_index     TYPE i DEFAULT 0
        ii_custom_mapping  TYPE REF TO zoblomov_if_ajson_mapping OPTIONAL
        is_opts            TYPE zoblomov_if_ajson=>ty_opts OPTIONAL
        iv_item_order      TYPE i DEFAULT 0
      RETURNING
        VALUE(rt_nodes)   TYPE zoblomov_if_ajson_types=>ty_nodes_tt
      RAISING
        zoblomov_cx_ajson_error.
    ...
    CLASS-METHODS insert_with_type
      IMPORTING
        iv_data            TYPE any
        iv_type            TYPE zoblomov_if_ajson_types=>ty_node_type
        is_prefix          TYPE zoblomov_if_ajson_types=>ty_path_name OPTIONAL
        iv_array_index     TYPE i DEFAULT 0
        ii_custom_mapping  TYPE REF TO zoblomov_if_ajson_mapping OPTIONAL
        is_opts            TYPE zoblomov_if_ajson=>ty_opts OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    METHODS convert_any
      IMPORTING
        ...
        is_prefix TYPE zoblomov_if_ajson_types=>ty_path_name
        iv_index TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
        ct_nodes TYPE zoblomov_if_ajson_types=>ty_nodes_tt
      RAISING
        zoblomov_cx_ajson_error.

    METHODS convert_ajson
      IMPORTING
        io_json TYPE REF TO zoblomov_if_ajson
        is_prefix TYPE zoblomov_if_ajson_types=>ty_path_name
        iv_index TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
...
CLASS lcl_abap_to_json IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    DATA lo_dummy TYPE REF TO zoblomov_cl_ajson.
        ...
        ELSEIF io_type->type_kind = lif_kind=>object_ref
          ...
          convert_ajson(
            ...
            CHANGING
              ct_nodes = ct_nodes ).
        ELSE.
          zoblomov_cx_ajson_error=>raise( |Unsupported type [{ io_type->type_kind
  ...
  METHOD convert_value.
    ...
    IF io_type->absolute_name = '\TYPE-POOL=ABAP\TYPE=ABAP_BOOL'
      ...
      ls_node-type = zoblomov_if_ajson_types=>node_type-boolean.
    ...
    ELSEIF io_type->absolute_name = '\TYPE=TIMESTAMP'.
      IF mv_format_datetime = abap_true.
        ls_node-type  = zoblomov_if_ajson_types=>node_type-string.
        ls_node-value = format_timestamp( iv_data ).
      ELSE.
        ls_node-type  = zoblomov_if_ajson_types=>node_type-number.
    ...
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      ...
      ls_node-value = |{ iv_data }|.
    ELSE.
      zoblomov_cx_ajson_error=>raise( |Unexpected elementary type [{
  ...
  METHOD convert_ref.
    ...
    IF iv_data IS INITIAL.
      ls_node-type  = zoblomov_if_ajson_types=>node_type-null.
      ls_node-value = 'null'.
    ELSE.
      " TODO support data references
      zoblomov_cx_ajson_error=>raise( |Unexpected reference @{ is_prefix-path && is_prefix-name }| ).
    ENDIF.
  ...
  METHOD convert_struc.
    ...
    ls_root-type  = zoblomov_if_ajson_types=>node_type-object.
  ...
  METHOD convert_table.
    ...
    ls_root-path  = is_prefix-path.
    ls_root-name  = is_prefix-name.
    ls_root-type  = zoblomov_if_ajson_types=>node_type-array.
    ls_root-index = iv_index.
    ls_root-order = iv_item_order.
  ...
  METHOD insert_value_with_type.
    ...
    IF io_type->type_kind CO lif_kind=>texts OR
      ...
      IF iv_type = zoblomov_if_ajson_types=>node_type-boolean AND iv_data <> 'true' AND iv_data <> 'false'.
        zoblomov_cx_ajson_error=>raise( |Unexpected boolean value [{ iv_data }] @{ lv_prefix }| ).
      ELSEIF iv_type = zoblomov_if_ajson_types=>node_type-null AND iv_data IS NOT INITIAL.
        zoblomov_cx_ajson_error=>raise( |Unexpected null value [{ iv_data }] @{ lv_prefix }| ).
      ELSEIF iv_type = zoblomov_if_ajson_types=>node_type-number AND iv_data CN '0123456789. E+-'.
        zoblomov_cx_ajson_error=>raise( |Unexpected numeric value [{ iv_data }] @{ lv_prefix }| ).
      ELSEIF iv_type <> zoblomov_if_ajson_types=>node_type-string AND iv_type <> zoblomov_if_ajson_types=>node_type-boolean
        AND iv_type <> zoblomov_if_ajson_types=>node_type-null AND iv_type <> zoblomov_if_ajson_types=>node_type-number.
        zoblomov_cx_ajson_error=>raise( |Unexpected type for value [{ iv_type },{ iv_data }] @{ lv_prefix }| ).
      ENDIF.
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      IF iv_type <> zoblomov_if_ajson_types=>node_type-number.
        zoblomov_cx_ajson_error=>raise( |Unexpected value for numeric [{ iv_data }] @{ lv_prefix }| ).
      ENDIF.
    ELSE.
      zoblomov_cx_ajson_error=>raise( |Unexpected type [{ io_type->type_kind }] @{ lv_prefix }| ).
    ENDIF.
...
INTERFACE lif_mutator_runner.
  METHODS run
    IMPORTING
      it_source_tree TYPE zoblomov_if_ajson_types=>ty_nodes_ts
    EXPORTING
      et_dest_tree TYPE zoblomov_if_ajson_types=>ty_nodes_ts
    RAISING
      zoblomov_cx_ajson_error.
ENDINTERFACE.
...
CLASS lcl_filter_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_filter TYPE REF TO zoblomov_if_ajson_filter
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_filter TYPE REF TO zoblomov_if_ajson_filter.
    DATA mr_source_tree TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.

    METHODS walk
      IMPORTING
        iv_path TYPE string
      CHANGING
        cs_parent TYPE zoblomov_if_ajson_types=>ty_node OPTIONAL
      RAISING
        zoblomov_cx_ajson_error.
...
CLASS lcl_filter_runner IMPLEMENTATION.
  ...
  METHOD walk.
    ...
    DATA ls_node TYPE zoblomov_if_ajson_types=>ty_node.
    DATA lv_tab_key TYPE string.

    IF cs_parent-type = zoblomov_if_ajson_types=>node_type-array.
      lv_tab_key = 'array_index'. " path + index
    ENDIF.

    LOOP AT mr_source_tree->* INTO ls_node USING KEY (lv_tab_key) WHERE path = iv_path.
      CASE ls_node-type.
        WHEN zoblomov_if_ajson_types=>node_type-boolean OR zoblomov_if_ajson_types=>node_type-null
          OR zoblomov_if_ajson_types=>node_type-number OR zoblomov_if_ajson_types=>node_type-string.

          IF mi_filter->keep_node( ls_node ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN zoblomov_if_ajson_types=>node_type-array OR zoblomov_if_ajson_types=>node_type-object.

          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zoblomov_if_ajson_filter=>visit_type-open ) = abap_false.
            CONTINUE.
          ENDIF.
          ...
          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zoblomov_if_ajson_filter=>visit_type-close ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN OTHERS.
          zoblomov_cx_ajson_error=>raise( |Unexpected node type { ls_node-type }| ).
      ENDCASE.
...
CLASS lcl_mapper_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_mapper TYPE REF TO zoblomov_if_ajson_mapping
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_mapper TYPE REF TO zoblomov_if_ajson_mapping.
    DATA mr_source_tree TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.
    ...
    METHODS process_deep_node
      IMPORTING
        ...
        iv_node_type    TYPE zoblomov_if_ajson_types=>ty_node-type
      RAISING
        zoblomov_cx_ajson_error.
...
CLASS lcl_mapper_runner IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    IF sy-subrc <> 0
      OR NOT ( <root>-type = zoblomov_if_ajson_types=>node_type-array OR <root>-type = zoblomov_if_ajson_types=>node_type-object ).
  ...
  METHOD process_deep_node.
    ...
    LOOP AT mr_source_tree->* ASSIGNING <item> WHERE path = iv_path.
      ...
      IF iv_node_type <> zoblomov_if_ajson_types=>node_type-array.
      ...
      IF <item>-type = zoblomov_if_ajson_types=>node_type-array OR <item>-type = zoblomov_if_ajson_types=>node_type-object.
        process_deep_node(
...
CLASS lcl_mutator_queue IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    DATA lr_buf TYPE REF TO zoblomov_if_ajson_types=>ty_nodes_ts.
