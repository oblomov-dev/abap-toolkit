...
CLASS lcl_empty_filter IMPLEMENTATION.
  METHOD zoblomov_if_ajson_filter~keep_node.
    ...
    rv_keep = boolc(
      ( iv_visit = zoblomov_if_ajson_filter=>visit_type-value AND is_node-value IS NOT INITIAL ) OR
      ( iv_visit <> zoblomov_if_ajson_filter=>visit_type-value AND is_node-children > 0 ) ).
...
CLASS lcl_paths_filter IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF boolc( iv_skip_paths IS INITIAL ) = boolc( it_skip_paths IS INITIAL ). " XOR
      zoblomov_cx_ajson_error=>raise( 'no filter path specified' ).
    ENDIF.
...
CLASS lcl_and_filter DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        it_filters TYPE zoblomov_if_ajson_filter=>ty_filter_tab
      RAISING
        zoblomov_cx_ajson_error.
  PRIVATE SECTION.
    DATA mt_filters TYPE zoblomov_if_ajson_filter=>ty_filter_tab.
ENDCLASS.
