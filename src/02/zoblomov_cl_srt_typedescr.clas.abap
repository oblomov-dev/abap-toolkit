...
CLASS zoblomov_cl_srt_typedescr DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create_by_rtti
      IMPORTING
        !rtti TYPE REF TO cl_abap_typedescr
      RETURNING
        VALUE(srtti) TYPE REF TO zoblomov_cl_srt_typedescr.
...
CLASS zoblomov_cl_srt_typedescr IMPLEMENTATION.
  METHOD constructor.
  ...
  METHOD create_by_rtti.
    ...
    CASE rtti->kind.
      WHEN cl_abap_typedescr=>kind_elem.
        ...
        elem_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_elemdescr
          EXPORTING rtti = elem_rtti.

      WHEN cl_abap_typedescr=>kind_struct.

        struct_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_structdescr
          EXPORTING rtti = struct_rtti.

      WHEN cl_abap_typedescr=>kind_table.

        table_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_tabledescr
          EXPORTING rtti = table_rtti.

      WHEN cl_abap_typedescr=>kind_ref.

        ref_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_refdescr
          EXPORTING rtti = ref_rtti.

      WHEN cl_abap_typedescr=>kind_class.

        class_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_classdescr
          EXPORTING rtti = class_rtti.

      WHEN cl_abap_typedescr=>kind_intf.

        intf_rtti ?= rtti.
        CREATE OBJECT srtti TYPE zoblomov_cl_srt_intfdescr
          EXPORTING rtti = intf_rtti.

      WHEN OTHERS.
        " Unsupported (new ABAP features in the future)
        RAISE EXCEPTION TYPE zoblomov_cx_srt.
    ENDCASE.
  ENDMETHOD.
