...
CLASS ltcl_test IMPLEMENTATION.
  METHOD test_func_get_uuid_32.
    ...
    DATA(lv_uuid) = zoblomov_cl_util_abap=>uuid_get_c32( ).
  ...
  METHOD test_func_get_uuid_22.
    ...
    DATA(lv_uuid) = zoblomov_cl_util_abap=>uuid_get_c22( ).
    cl_abap_unit_assert=>assert_not_initial( lv_uuid ).
    cl_abap_unit_assert=>assert_equals( exp = strlen( lv_uuid )
                                        act = 22 ).
  ...
  METHOD test_encoding.
    ...
    DATA(lv_xstring)  = zoblomov_cl_util_abap=>conv_get_xstring_by_string( lv_string ).
    DATA(lv_string2)  = zoblomov_cl_util_abap=>conv_encode_x_base64( lv_xstring ).
    DATA(lv_xstring2) = zoblomov_cl_util_abap=>conv_decode_x_base64( lv_string2 ).
    DATA(lv_string3)  = zoblomov_cl_util_abap=>conv_get_string_by_xstring( lv_xstring2 ).
  ...
  METHOD test_classes_impl_intf.
    ...
    DATA(mt_classes) = zoblomov_cl_util_abap=>rtti_get_classes_impl_intf( `IF_SERIALIZABLE_OBJECT`  ).
