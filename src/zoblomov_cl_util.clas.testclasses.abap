...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_sql_get_by_string.
    ...
    DATA(lv_test) = ``.
    DATA(ls_sql) = zoblomov_cl_util=>filter_get_sql_by_sql_string( lv_test ) ##NEEDED.

  ENDMETHOD.

  METHOD test_create.

    DATA(lo_test) = NEW zoblomov_cl_util( ) ##NEEDED.
  ...
  METHOD test_time_get_timestampl.
    ...
    DATA(lv_time) = zoblomov_cl_util=>time_get_timestampl( ).

    DATA(lv_time2) = zoblomov_cl_util=>time_substract_seconds( time    = lv_time
                                                            seconds = 60 * 60 * 4 ).
  ...
  METHOD test_time_substract_seconds.
    ...
    DATA(lv_time) = zoblomov_cl_util=>time_get_timestampl( ).
    DATA(lv_time2) = zoblomov_cl_util=>time_get_timestampl( ).
  ...
  METHOD test_x_check_raise.
    ...
    TRY.
        zoblomov_cl_util=>x_check_raise( xsdbool( 1 = 1 ) ).
        cl_abap_unit_assert=>fail( ).
      CATCH cx_root.
    ENDTRY.

    TRY.
        zoblomov_cl_util=>x_check_raise( xsdbool( 1 = 3 ) ).
      CATCH cx_root.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ...
  METHOD test_url_param_create_url.
    ...
    DATA(lt_param) = zoblomov_cl_util=>url_param_get_tab(
                         `https://url.com/rvice_for_ui?sap-client=100&app_start=z2ui5_cl_app_hello_world` ).
    DATA(lv_url) = zoblomov_cl_util=>url_param_create_url( lt_param ).
  ...
  METHOD test_url_param_get.
    ...
    DATA(lv_param) = zoblomov_cl_util=>url_param_get(
                         val = `app_start`
  ...
  METHOD test_url_param_set.
    ...
    DATA(lv_param) = zoblomov_cl_util=>url_param_set(
                         name  = `app_start`
  ...
  METHOD test_x_check_raise_not.
    ...
    TRY.
        zoblomov_cl_util=>x_check_raise( xsdbool( 1 = 2 ) ).
  ...
  METHOD test_rtti_get_type_name.
    ...
    DATA(lv_name) = zoblomov_cl_util=>rtti_get_type_name( lv_xsdbool ).
  ...
  METHOD test_rtti_get_type_kind.
    ...
    DATA(lv_type_kind) = zoblomov_cl_util=>rtti_get_type_kind( lv_string ).
  ...
  METHOD test_rtti_get_t_attri_by_obj.
    ...
    DATA(lt_attri) = zoblomov_cl_util=>rtti_get_t_attri_by_oref( lo_obj ).
  ...
  METHOD test_rtti_get_t_comp_by_struc.
    ...
    DATA(lt_comp) = zoblomov_cl_util=>rtti_get_t_attri_by_any( ls_row ).
  ...
  METHOD test_trans_xml_any_2__w_obj.
    ...
    DATA(lo_obj) = NEW ltcl_test_app( ).
    DATA(lv_xml) = zoblomov_cl_util=>xml_stringify( lo_obj ).
  ...
  METHOD test_trans_xml_any_2__w_data.
    ...
    DATA(lv_xml) = zoblomov_cl_util=>xml_stringify( ls_row ).
  ...
  METHOD test_get_token_t_by_r_t.
    ...
    DATA(lt_range) = VALUE zoblomov_cl_util=>ty_t_range( ( sign = 'I' option = 'EQ' low = `table` high = `` )
     ).

    DATA(lt_result) = zoblomov_cl_util=>filter_get_token_t_by_range_t( lt_range ).

    DATA(lt_exp) = VALUE zoblomov_cl_util=>ty_t_token(
