...
CLASS zoblomov_cl_util IMPLEMENTATION.
  ...
  METHOD filter_update_tokens.
    ...
    DATA(lt_range) = zoblomov_cl_util=>filter_get_range_t_by_token_t( result[ name = name ]-t_token ).
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST zoblomov_if_ajson( zoblomov_cl_ajson=>create_empty( ) ).
...
*                      v = c_trim_lower( lv_value ) ) INTO TABLE rt_params.
  ...
  METHOD filter_get_sql_where.
    ...
    LOOP AT val INTO DATA(ls_filter).
      ...
      DATA(lo_range) = NEW zoblomov_cl_util_range( iv_fieldname = ls_filter-name
