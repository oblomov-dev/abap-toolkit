CLASS zcl_pop_file_dl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)       TYPE REF TO zcl_pop_file_dl.
...
CLASS zcl_pop_file_dl IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    DATA(popup) = zcl_xml_view=>factory_popup( )->dialog(
                  title       = title
                  icon        = icon
    ...
    IF mv_check_download = abap_true.
      DATA(lv_csv_x) = zcl_util=>conv_get_xstring_by_string( mv_value ).
      DATA(lv_base64) = zcl_util=>conv_encode_x_base64( lv_csv_x ).
