CLASS zcl_pop_to_select DEFINITION
  PUBLIC
  FINAL
  CREATE PROTECTED .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result)     TYPE REF TO zcl_pop_to_select .
    METHODS result
      RETURNING
...
CLASS zcl_pop_to_select IMPLEMENTATION.
  ...
  METHOD factory.
    ...
    r_result->mr_tab = zcl_util=>conv_copy_ref_data( i_tab ).
  ...
  METHOD display.
    ...
    DATA(lt_comp) = zcl_util=>rtti_get_t_attri_by_any( <tab_out> ).
    ...
    LOOP AT lt_comp INTO ls_comp.
      DATA(text) = COND #(
                     LET data_element_name = substring_after( val = CAST cl_abap_elemdescr( ls_comp-type )->absolute_name sub = '\TYPE=' )
                         medium_label = zcl_util=>rtti_get_data_element_texts( data_element_name )-medium IN
