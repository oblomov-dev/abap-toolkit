CLASS zcl_xml_view_cc DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        !view TYPE REF TO zcl_xml_view .

  PROTECTED SECTION.
    DATA mo_view TYPE REF TO zcl_xml_view.
  PRIVATE SECTION.
ENDCLASS.



CLASS zcl_xml_view_cc IMPLEMENTATION.
  ...
  METHOD camera_picture.
    ...
    mo_view->_generic( name = `CameraPicture`
              ...
              t_prop        = VALUE #(
                ...
                ( n = `press`    v = press )
                ( n = `OnPhoto`    v = onphoto )
                ( n = `autoplay` v = zcl_util=>boolean_abap_2_json( autoplay ) )
       ) ).
  ...
  METHOD file_uploader.
    ...
    mo_view->_generic( name = `FileUploader`
              ...
              t_prop        = VALUE #( (  n = `placeholder`        v = placeholder )
                                ...
                                (  n = `path`               v = path )
                                (  n = `value`              v = value )
                                (  n = `iconOnly`           v = zcl_util=>boolean_abap_2_json( icononly ) )
                                (  n = `buttonOnly`         v = zcl_util=>boolean_abap_2_json( buttononly ) )
                                (  n = `buttonText`         v = buttontext )
                                (  n = `uploadButtonText`   v = uploadbuttontext )
                                (  n = `fileType`           v = filetype )
                                (  n = `checkDirectUpload`  v = zcl_util=>boolean_abap_2_json( checkdirectupload ) )
                                (  n = `icon`           v = icon )
  ...
  METHOD geolocation.
    ...
    mo_view->_generic( name = `Geolocation`
              ...
              t_prop        = VALUE #(
                    ...
                    ( n = `speed`  v = speed )
                    ( n = `heading`  v = heading )
                    ( n = `enableHighAccuracy`  v = zcl_util=>boolean_abap_2_json( enablehighaccuracy ) )
                    ( n = `timeout`  v = timeout )
              ) ).
  ...
  METHOD timer.
    ...
    mo_view->_generic( name = `Timer`
              ...
              t_prop        = VALUE #( ( n = `delayMS`  v = delayms )
                                ( n = `finished`  v = finished )
                                ( n = `checkActive`  v = zcl_util=>boolean_abap_2_json( checkactive ) )
                                ( n = `checkRepeat`  v = zcl_util=>boolean_abap_2_json( checkrepeat ) )
              ) ).
...
*                                ( n = `checkRepeat`  v = z2ui5_cl_util=>boolean_abap_2_json( checkrepeat ) )
  ...
  METHOD dirty.
    ...
    mo_view->_generic( name = `Dirty`
              ns            = `z2ui5`
              t_prop        = VALUE #( ( n = `isDirty`  v = zcl_util=>boolean_abap_2_json( isdirty ) ) ) ).
