CLASS zcl_core_app DEFINITION
  ...
  PUBLIC SECTION.
    ...
    DATA ms_draft   TYPE zif_types=>ty_s_get-s_draft.
    ...
    CLASS-METHODS all_xml_parse
      IMPORTING
        !xml          TYPE clike
      RETURNING
        VALUE(result) TYPE REF TO zcl_core_app.
...
CLASS zcl_core_app IMPLEMENTATION.
  ...
  METHOD all_xml_stringify.
    ...
    TRY.
        ...
        result = zcl_util=>xml_stringify( me ).
      ...
      CATCH cx_root INTO DATA(x2).
        TRY.
            ...
            DATA(lo_dissolver) = NEW zcl_core_diss_srv(
              attri = mt_attri
              app   = mo_app ).

            lo_dissolver->main( ).
            lo_dissolver->main( ).
            lo_model = NEW zcl_core_attri_srv(
              attri = mt_attri
              app   = mo_app ).
            lo_model->attri_before_save( ).
  ...
  METHOD db_load.
    ...
    DATA(lo_db) = NEW zcl_core_draft_srv( ).
  ...
  METHOD db_save.
    ...
    IF mo_app IS BOUND.
      CAST zif_app( mo_app )->id_draft = ms_draft-id.
    ENDIF.
  ...
  METHOD model_json_parse.
    ...
    DATA(lo_json_mapper) = NEW zcl_core_json_srv( ).
