CLASS zcl_core_http_post DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA ms_request      TYPE zif_core_types=>ty_s_http_request_post.
    DATA ms_response     TYPE zif_core_types=>ty_s_http_response_post.
...
CLASS zcl_core_http_post IMPLEMENTATION.


  METHOD constructor.

    mv_request_json = val.
    mo_action = NEW zcl_core_action( me ).
  ...
  METHOD main_end.
    ...
    ms_response = VALUE #(
        ...
        s_front-app    = zcl_util=>rtti_get_classname_by_ref( mo_action->mo_app->mo_app )
        ).
  ...
  METHOD main_process.
    TRY.
        ...
        DATA(li_client) = NEW zcl_core_client( mo_action ).
        DATA(li_app)    = CAST zif_app( mo_action->mo_app->mo_app ).
