...
CLASS zcl_core_http_get DEFINITION LOCAL FRIENDS ltcl_test_http_get.

CLASS ltcl_test_http_get IMPLEMENTATION.

  METHOD file_not_initial.

    DATA(lo_get) = NEW zcl_core_http_get( ).
...
*    ENDIF.
  ...
  METHOD debugging_tools.
    ...
    DATA(lv_index_html) = to_upper( zcl_cc_debug_tool=>get_js( ) ) ##NEEDED.
    IF lv_index_html CS `<`.
