...
CLASS zcl_core_http_post DEFINITION LOCAL FRIENDS ltcl_test_handler_post.
...
CLASS ltcl_test_handler_post IMPLEMENTATION.
  ...
  METHOD load_startup_app.
    ...
    DATA(lo_post) = NEW zcl_core_http_post( lv_payload ).
    lo_post->main_begin( ).
    ...
    DATA(lo_startup) = CAST zcl_core_app_startup( lo_post->mo_action->mo_app->mo_app ) ##NEEDED.
