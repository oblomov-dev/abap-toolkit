CLASS zcl_core_http_get DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA ms_request TYPE zif_types=>ty_s_http_request_get.
  ...
  PROTECTED SECTION.
    ...
    METHODS get_default_config
      RETURNING
        VALUE(result) TYPE zif_types=>ty_s_http_request_get-t_config.
...
CLASS zcl_core_http_get IMPLEMENTATION.
...
*        (  n = `src`                       v = `https://ui5.sap.com/1.124.0/resources/sap-ui-core.js` )
  ...
  METHOD get_js.
    ...
    DATA(lv_two_way_model) = zif_core_types=>cs_ui5-two_way_model.
...
`he App" }).open();  } } ) }` && |\n| &&
  ...
  METHOD get_js_cc_startup.
    ...
    result = ` ` &&
        ...
        zcl_cc_focus=>get_js( ) &&
        zcl_cc_title=>get_js( ) &&
        zcl_cc_lp_title=>get_js( ) &&
        zcl_cc_history=>get_js( ) &&
        zcl_cc_scrolling=>get_js( ) &&
        zcl_cc_info=>get_js( ) &&
        zcl_cc_geoloc=>get_js( ) &&
        zcl_cc_file_upl=>get_js( ) &&
        zcl_cc_multiinput=>get_js( ) &&
        zcl_cc_uitable=>get_js( ) &&
        zcl_cc_util=>get_js( ) &&
        zcl_cc_favicon=>get_js( ) &&
        zcl_cc_dirty=>get_js( ) &&
       `  `.
  ...
  METHOD main.
    ...
    mv_response = mv_response &&
       zcl_cc_debug_tool=>get_js( ).

    mv_response = mv_response && |\n| &&
                 `</script>` && |\n| &&
                 `<abc/></body></html>`.

    NEW zcl_core_draft_srv( )->cleanup( ).
    result = mv_response.
