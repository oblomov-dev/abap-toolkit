...
CLASS zcl_core_app DEFINITION LOCAL FRIENDS ltcl_test.

CLASS ltcl_test IMPLEMENTATION.

  METHOD first_test.

    DATA(lo_action) = NEW zcl_core_app( ) ##NEEDED.
...
CLASS ltcl_test_db IMPLEMENTATION.
  ...
  METHOD test_db_save.
    ...
    DATA(lo_app) = NEW zcl_core_app( ).
    lo_app->ms_draft-id = `TEST_ID`.
    lo_app->mo_app = lo_app_user.

    lo_app->db_save( ).

    DATA(lo_app_db) = zcl_core_app=>db_load( `TEST_ID` ).
