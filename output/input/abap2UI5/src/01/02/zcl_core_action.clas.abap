CLASS zcl_core_action DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA mo_http_post TYPE REF TO zcl_core_http_post.
    DATA mo_app       TYPE REF TO zcl_core_app.

    DATA ms_actual TYPE zif_core_types=>ty_s_actual.
    DATA ms_next   TYPE zif_core_types=>ty_s_next.

    METHODS factory_system_startup
      RETURNING
        VALUE(result) TYPE REF TO zcl_core_action.
    ...
    METHODS constructor
      IMPORTING
        val TYPE REF TO zcl_core_http_post.

  PROTECTED SECTION.

    METHODS prepare_app_stack
      IMPORTING
        val           TYPE zif_core_types=>ty_s_next-o_app_leave
      RETURNING
        VALUE(result) TYPE REF TO zcl_core_action.
...
CLASS zcl_core_action IMPLEMENTATION.
  ...
  METHOD factory_by_frontend.
    ...
    result->mo_app = zcl_core_app=>db_load( mo_http_post->ms_request-s_front-id ).

    result->mo_app->ms_draft-id      = zcl_util=>uuid_get_c32( ).
  ...
  METHOD factory_first_start.
    ...
    TRY.
        ...
        DATA(li_app) = CAST zif_app( result->mo_app->mo_app ).
  ...
  METHOD factory_stack_leave.
    ...
    "check for new app?
    TRY.
        DATA(lo_draft) = NEW zcl_core_draft_srv( ).
  ...
  METHOD factory_system_error.
    ...
    result->mo_app->ms_draft-id          = zcl_util=>uuid_get_c32( ).
    result->ms_actual-check_on_navigated = abap_true.
    result->ms_next-o_app_call           = zcl_core_app_error=>factory( ix ).
  ...
  METHOD factory_system_startup.
    ...
    result->mo_app->ms_draft-id          = zcl_util=>uuid_get_c32( ).
    result->ms_actual-check_on_navigated = abap_true.
    result->mo_app->mo_app               = zcl_core_app_startup=>factory( ).
  ...
  METHOD prepare_app_stack.
    ...
    val->id_draft = COND string( WHEN val->id_draft IS INITIAL
        THEN zcl_util=>uuid_get_c32( )
        ELSE ms_next-o_app_leave->id_draft ).

    result = NEW #( mo_http_post ).
    TRY.
        result->mo_app = zcl_core_app=>db_load_by_app( val ).
      CATCH cx_root.
