CLASS zcl_core_bind_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA mo_app    TYPE REF TO zcl_core_app .
    DATA mr_attri  TYPE REF TO zif_core_types=>ty_s_attri .
    DATA ms_config TYPE zif_core_types=>ty_s_bind_config .
    DATA mv_type   TYPE string .

    METHODS constructor
      IMPORTING
        !app TYPE REF TO zcl_core_app .

    METHODS main_local
      IMPORTING
        !val          TYPE data
        !config       TYPE zif_core_types=>ty_s_bind_config OPTIONAL
      RETURNING
...
CLASS zcl_core_bind_srv IMPLEMENTATION.
  ...
  METHOD bind_tab_cell.
    ...
    DATA(lt_attri) = zcl_util=>rtti_get_t_attri_by_any( ms_config-tab ).
  ...
  METHOD check_raise_existing.
    ...
    IF mr_attri->custom_mapper IS BOUND.
      ...
      DATA(lv_name1) = zcl_util=>rtti_get_classname_by_ref( mr_attri->custom_mapper ).
      DATA(lv_name2) = zcl_util=>rtti_get_classname_by_ref( ms_config-custom_mapper ).
  ...
  METHOD get_client_name.
    ...
    result = COND #( WHEN mv_type = zif_core_types=>cs_bind_type-two_way
        THEN `/` && zif_core_types=>cs_ui5-two_way_model )
        && `/` && result.
  ...
  METHOD main.
    ...
    IF zcl_util=>check_bound_a_not_inital( config-tab ).
    ...
    DATA(lo_model) = NEW zcl_core_attri_srv(
    ...
    IF `/` && zif_core_types=>cs_ui5-two_way_model = result.
      RAISE EXCEPTION TYPE zcx_util_error
        EXPORTING
  ...
  METHOD main_cell.
    ...
    DATA(lo_bind) = NEW zcl_core_bind_srv( mo_app ).
  ...
  METHOD main_local.
    TRY.
        ...
        DATA(lo_json) = CAST zif_ajson( zcl_ajson=>new( ) ).
        lo_json->set( iv_path = `/` iv_val = val ).

        IF config-custom_mapper IS BOUND.
          lo_json = lo_json->map( config-custom_mapper ).
        ELSE.
          lo_json = lo_json->map( zcl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( zcl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( zcl_util=>uuid_get_c22( ) ).
        INSERT VALUE #( name_client     = |/{ lv_id }|
                        name            = lv_id
                        json_bind_local = lo_json
                        bind_type       = zif_core_types=>cs_bind_type-one_time )
  ...
  METHOD update_model_attri.
    ...
    mr_attri->view          = COND #( WHEN ms_config-view IS INITIAL THEN zif_client=>cs_view-main ELSE ms_config-view ).
