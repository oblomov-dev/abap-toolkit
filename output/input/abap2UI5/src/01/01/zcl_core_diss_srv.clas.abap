CLASS zcl_core_diss_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        !attri TYPE REF TO zif_core_types=>ty_t_attri
        !app   TYPE REF TO object.

    METHODS main .
  PROTECTED SECTION.

    DATA mt_attri TYPE REF TO zif_core_types=>ty_t_attri.
    ...
    METHODS diss_struc
      IMPORTING
        ir_attri      TYPE REF TO zif_core_types=>ty_s_attri
      RETURNING
        VALUE(result) TYPE zif_core_types=>ty_t_attri.
    ...
    METHODS diss_oref
      IMPORTING ir_attri      TYPE REF TO zif_core_types=>ty_s_attri
      RETURNING VALUE(result) TYPE zif_core_types=>ty_t_attri.

    METHODS create_new_entry
      IMPORTING
        name          TYPE string
      RETURNING
        VALUE(result) TYPE zif_core_types=>ty_s_attri.
...
CLASS zcl_core_diss_srv IMPLEMENTATION.
  ...
  METHOD create_new_entry.
    ...
    result  = VALUE zif_core_types=>ty_s_attri( ).
    result-name        = name.
  ...
  METHOD diss_dref.
    ...
    IF zcl_util=>check_unassign_inital( ir_attri->r_ref ).
      RETURN.
    ENDIF.

    DATA(lr_ref) = zcl_util=>unassign_data( ir_attri->r_ref ).
    IF lr_ref IS INITIAL.
      RETURN.
    ENDIF.

    DATA(ls_attri2) = VALUE zif_core_types=>ty_s_attri( ).
  ...
  METHOD diss_oref.
    ...
    DATA(lr_ref) = zcl_util=>unassign_object( ir_attri->r_ref ).
    DATA(lt_attri) = zcl_util=>rtti_get_t_attri_by_oref( lr_ref ).
  ...
  METHOD diss_struc.
    ...
    DATA(lt_attri) = zcl_util=>rtti_get_t_attri_by_any( lr_ref ).
  ...
  METHOD main_init.
    ...
    IF mt_attri->* IS NOT INITIAL.
      LOOP AT mt_attri->* TRANSPORTING NO FIELDS
        WHERE bind_type <> zif_core_types=>cs_bind_type-one_time.
      ENDLOOP.
      IF sy-subrc = 0.
        RETURN.
      ENDIF.
    ...
    DATA(ls_attri) = VALUE zif_core_types=>ty_s_attri( r_ref = REF #( mo_app ) ).
  ...
  METHOD main_run.
    ...
    DATA(lt_attri_new) = VALUE zif_core_types=>ty_t_attri( ).

    LOOP AT mt_attri->* REFERENCE INTO DATA(lr_attri)
         WHERE check_dissolved = abap_false
         AND bind_type <> zif_core_types=>cs_bind_type-one_time.
