CLASS zcl_core_draft_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        !draft     TYPE zif_types=>ty_s_draft
    ...
    METHODS read_draft
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE zif_core_types=>ty_s_db.

    METHODS read_info
      IMPORTING
        !id           TYPE clike
      RETURNING
        VALUE(result) TYPE zif_types=>ty_s_draft.
...
CLASS zcl_core_draft_srv IMPLEMENTATION.


  METHOD cleanup.

    DATA(lv_four_hours_ago) = zcl_util=>time_substract_seconds(
        time    = zcl_util=>time_get_timestampl( )
        seconds = 60 * 60 * 4 ).

    DELETE FROM zt_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE zif_core_types=>ty_s_db(
        id                = draft-id
        id_prev           = draft-id_prev
        id_prev_app       = draft-id_prev_app
        id_prev_app_stack = draft-id_prev_app_stack
        uname             = zcl_util=>context_get_user_tech( )
        timestampl        = zcl_util=>time_get_timestampl( )
        data              = model_xml ).

    MODIFY zt_01 FROM @ls_db.
    IF sy-subrc <> 0.
