CLASS zcl_core_app_info DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    DATA mv_device_gav TYPE string.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO zcl_core_app_info.
...
CLASS zcl_core_app_info IMPLEMENTATION.
  ...
  METHOD view_display_start.
    ...
    DATA(page2) = zcl_xml_view=>factory_popup(
         )->dialog(
...
*   simple_form2->text( client->_bind( mv_device_type ) ).
    ...
    simple_form2->checkbox( enabled = abap_false selected = zcl_util=>context_check_abap_cloud( ) ).

    DATA(lv_count) = CONV string( NEW zcl_core_draft_srv( )->count_entries( ) ).
    simple_form2->toolbar( )->title( `abap2UI5` ).
    simple_form2->label( `Version ` ).
    simple_form2->text( zif_app=>version ).
