CLASS zcl_core_app_startup DEFINITION
  PUBLIC
  FINAL
  CREATE PROTECTED .
  ...
  PUBLIC SECTION.
    ...
    DATA client TYPE REF TO zif_client.
    DATA mv_check_initialized TYPE abap_bool.

    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO zcl_core_app_startup.
  ...
  PROTECTED SECTION.
    DATA mt_classes TYPE zcl_util=>ty_t_classes.
...
CLASS zcl_core_app_startup IMPLEMENTATION.
  ...
  METHOD on_event_check.
    DATA li_app_test TYPE REF TO zif_app.

    TRY.
        ms_home-classname = zcl_util=>c_trim_upper( ms_home-classname ).
        ...
        ms_home-url = zcl_core_util_srv=>app_get_url(
                   client    = client
  ...
  METHOD view_display_start.
    ...
    DATA(page2) = zcl_xml_view=>factory( )->shell( )->page(
    ...
    DATA(lv_url_samples2) = zcl_core_util_srv=>app_get_url(
                  client    = client
                  classname = 'z2ui5_cl_demo_app_000' ).

    simple_form2->toolbar( )->title( `What's next?` ).

    IF zcl_util=>rtti_check_class_exists( `z2ui5_cl_demo_app_000` ).
  ...
  METHOD zif_app~main.
    ...
    IF client->get( )-check_on_navigated = abap_true.
      TRY.
          DATA(lo_f4) = CAST zcl_pop_to_select( client->get_app( client->get( )-s_draft-id_prev_app ) ).
  ...
  METHOD z2ui5_on_event.
    ...
    DATA li_app TYPE REF TO zif_app.
    ...
    CASE client->get( )-event.
      ...
      WHEN `OPEN_INFO`.
        client->nav_app_call( zcl_core_app_info=>factory( ) ).
        RETURN.
      ...
      WHEN 'VALUE_HELP'.
        TRY.
            mt_classes = zcl_util=>rtti_get_classes_impl_intf( zcl_util=>rtti_get_intfname_by_ref( li_app ) ).
          CATCH cx_root.
            client->message_box_display( `Unfortunately the value help is not available on your ABAP release!` ).
            RETURN.
        ENDTRY.
        client->nav_app_call( zcl_pop_to_select=>factory( mt_classes ) ).
    ENDCASE.
  ...
  METHOD z2ui5_on_init.
    ...
    ms_home-class_editable = abap_true.
    ms_home-btn_icon       = `sap-icon://validate`.
    ms_home-classname      = zcl_util=>rtti_get_classname_by_ref( NEW zcl_core_app_hello_w( ) ).
